...
<p>
★★★★★★请点击后面的链接进行音频的生成，<a href="http://localhost:9874/" target="_blank">text to audio</a>.★★★★★★
</p>
</div>
"""
WEBSITE_bottom = """
...
def generate(
    ...
    # print("audio[1].shape", audio[1].shape)
    # print("audio[1]:", audio[1])
    cache_file_path = '/data2/jingyc/project1/cached/1822/audio/audio_2.wav'
    cache_file_path = os.path.join("/data2/jingyc/project1/cached/", f'{uid}','audio' , f"audio_{len(os.listdir(cached_dir)) + 1}.wav")
    # print(audio[1].dtype)
    print('audio:',type(audio))
    # 将音频数据写入缓存文件 测试，注释
    wav.write(cache_file_path, audio[0], audio[1])
    
    # with open(cache_file_path, 'wb') as f:
    #     f.write(audio)
    print("audio已保存至路径：", cache_file_path)
    audio_path = cache_file_path
    datas = []
    # print(audio.path)
    for r in range(repeat_times):
        ruid = random.randrange(999999999)
        ...
        args.outfile = pjoin('/data2/jingyc/project1/Wav2Lip/results/', "sample_repeat%d_%d.mp4"%(r, ruid) )
        run_inference1_command(args.checkpoint_path, args.face_path, cache_file_path, args.outfile, wav2lip_path)
        print(f"语音 '{audio_path}'已驱动并生成视频，保存到 '{args.outfile}'")
        video = args.outfile
...
def get_video_html(data, video_id, width=700, height=700):
    ...
    video_html = f"""
<h2 style='text-align: center'>
<a href="http://localhost:8867/{url}" download="video.mp4"><b>MP4 Download</b></a>
</h2>
...
autoplay loop disablepictureinpicture id="{video_id}">
  <source src="http://localhost:8867/{url}" type="video/mp4">
  Your browser does not support the video tag.
</video>
"""
    return video_html

def generate_component(generate_function, audio):
    if audio == "" or audio is None:
        return [None for _ in range(1)]
    # uid = random.randrange(99999)
    datas = generate_function(audio, uid)
...
with gr.Blocks(css=CSS, theme=theme) as demo:
    ...
    def clear_videos():
        return [None for x in range(1)] +[audio1]

    clear.click(fn=clear_videos, outputs=videos + [audio1])  #清除
